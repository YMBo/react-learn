...
if (process.env.NODE_ENV !== 'production') {
  (function() {
    ...
    let enableScopeAPI = false; // Experimental Create Event Handle API.
...

        {
          ...
          if (/^index\./.test(fileName)) {
            ...
            if (match) {
              ...
              if (pathBeforeSlash) {
                let folderName = pathBeforeSlash.replace(BEFORE_SLASH_RE, '');
      ...
      if (typeof type === 'object') {
        switch (type.$$typeof) {
          ...
          case REACT_LAZY_TYPE: {
            let thenable = type;
            let resolvedThenable = refineResolvedLazyComponent(thenable);
    ...
    function hasValidKey(config) {
      {
        if (hasOwnProperty.call(config, 'key')) {
          let getter = Object.getOwnPropertyDescriptor(config, 'key').get;
    ...
    let ReactCurrentOwner$1 = require('react/lib/ReactCurrentOwner');
    ...
    function getDeclarationErrorAddendum() {
      {
        if (ReactCurrentOwner$1.current) {
          let name = ReactCurrentOwner$1.current.getName();
    ...
    function validatePropTypes(element) {
      {
        ...
        if (propTypes) {
          ...
          let _name = getComponentName(type);
    ...
    function jsxWithValidation(
    ...
    ) {
      {
        ...
        if (!validType) {
          ...
          let sourceInfo = getSourceInfoErrorAddendum(source);
        ...
        let element = jsxDEV(type, props, key, source, self); // The result can be nullish if a mock or a custom function is used.
    ...
    let jsxDEV$1 = jsxWithValidation;
